...
export default function HomePage() {
  ...
  const financeTotals = useMemo(() => {
    ...
    const activeCreditTotal = credits.reduce(
      (sum, item) => (isCreditActive(item.paymentStartDate) ? sum + Number(item.monthlyPayment || 0) : sum),
      0
    );
  ...
  const investmentTotals = useMemo(() => {
    ...
    const goldCost = investments.gold.reduce((sum, item) => sum + Number(item.quantity || 0) * Number(item.buyPrice || 0), 0);
    const goldValue = investments.gold.reduce((sum, item) => sum + Number(item.quantity || 0) * Number(item.currentPrice || 0), 0);
    const cryptoCost = investments.crypto.reduce((sum, item) => sum + Number(item.quantity || 0) * Number(item.buyPrice || 0), 0);
    const cryptoValue = investments.crypto.reduce((sum, item) => sum + Number(item.quantity || 0) * Number(item.currentPrice || 0), 0);
    const blockedTotal = besTotal + oldLockedTotal;
  ...
  const updateIncome = (field, value) => {
    setIncome((current) => ({ ...current, [field]: formatNumberInput(value) }));
  };

  const updateExtraIncomeForm = (field, value) => {
    setExtraIncomeForm((current) => ({ ...current, [field]: field === "amount" ? formatNumberInput(value) : value }));
  };
  ...
  const updateCardForm = (field, value) => {
    setCardForm((current) => ({ ...current, [field]: field === "amount" ? formatNumberInput(value) : value }));
  };

  const updateOtherForm = (field, value) => {
    setOtherForm((current) => ({ ...current, [field]: field === "amount" ? formatNumberInput(value) : value }));
  };

  const updateGoldForm = (field, value) => {
    const amountFields = ["buyPrice", "currentPrice"];
    setGoldForm((current) => ({ ...current, [field]: amountFields.includes(field) ? formatNumberInput(value) : value }));
  };

  const updateCryptoForm = (field, value) => {
    const amountFields = ["buyPrice", "currentPrice"];
    setCryptoForm((current) => ({ ...current, [field]: amountFields.includes(field) ? formatNumberInput(value) : value }));
  };
  ...
  const addOrUpdateGold = () => {
    ...
    setInvestments((current) => ({
      ...current,
      gold: editingGoldId
        ? current.gold.map((item) => (item.id === editingGoldId ? { ...item, ...payload } : item))
  ...
  const addOrUpdateCrypto = () => {
    ...
    setInvestments((current) => ({
      ...
      crypto: editingCryptoId
        ? current.crypto.map((item) => (item.id === editingCryptoId ? { ...item, ...payload } : item))
    ...
    <main className="financePage">
      <div className="financeShell">
        ...
        <div className="tabBar">
          <button type="button" className={activeTab === "finance" ? "tabButton active" : "tabButton"} onClick={() => setActiveTab("finance")}>
            Gelir / Gider
          </button>
          <button type="button" className={activeTab === "investments" ? "tabButton active" : "tabButton"} onClick={() => setActiveTab("investments")}>
            Yatırımlar
          </button>
        </div>

        {dataLoading ? (
          <section className="panelCard"><div className="emptyState"><strong>Veriler yükleniyor...</strong></div></section>
        ) : null}
...
function AuthView(props) {
  const {
    authMode, setAuthMode, fullName, setFullName, email, setEmail, password, setPassword,
    passwordAgain, setPasswordAgain, pendingEmail, verificationCode, setVerificationCode,
    rememberMe, setRememberMe, authMessage, handleLogin, handleRegister, handleVerifyCode,
