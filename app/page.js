...
const goldTypeOptions = [
  ...
  { value: "GUMUS", label: "Gram Gümüş" },
  { value: "XAG", label: "Ons Gümüş" },
  { value: "XPT", label: "Ons Platin" },
  { value: "XPD", label: "Ons Paladyum" },
...
const DEFAULT_TABS = [
  ...
  { id: "admin", label: "Yönetim" }, // yalnız adminlere görünür
];
...
export default function HomePage() {
  ...
  // Ana bolumler acik baslar: kullanici sekmeye girdiginde bos ekran degil,
  // dogrudan verisini ve giris alanlarini gorur. Alt bolumler veri yuklenince
  // (loadFinanceData icinde) icinde kayit varsa kendiliginden acilir.
  ...
  const [extraIncomeForm, setExtraIncomeForm] = useState({ title: "", amount: "" });
  ...
  const toggleShowPnl = () =>
    setShowPnl((value) => {
      const next = !value;
      try {
        window.localStorage.setItem("finance-show-pnl", next ? "1" : "0");
      } catch {}
      return next;
    });
  ...
  useEffect(() => {
    ...
    // Admin mi? (profiles.is_admin — RLS yalnız kendi satırını verir)
    supabase
      .from("profiles")
      .select("is_admin")
  ...
  const loadFinanceData = async (userId) => {
    ...
    // Icinde kayit olan alt bolumleri acik getir: kullanici verisini gormek icin
    // ekstra tiklamak zorunda kalmasin. Bos olanlar kapali kalir (ekran sade).
    const dolu = (list) => Array.isArray(list) && list.length > 0;
    setExtraIncomeOpen(dolu(normalized.extraIncomes));
    setCreditsOpen(dolu(normalized.credits));
    setCardsOpen(dolu(normalized.cardExpenses));
    setOthersOpen(dolu(normalized.otherExpenses));
    setGoldOpen(dolu(normalized.investments?.gold));
    setStockOpen(dolu(normalized.investments?.stocks));
    setFundOpen(dolu(normalized.investments?.funds));
    setCryptoOpen(dolu(normalized.investments?.crypto));
    setForexOpen(dolu(normalized.investments?.forex));
  ...
  const handleGoogleLogin = async () => {
    ...
    if (error) setAuthMessage("Google ile giriş başlatılamadı: " + error.message);
  };
  ...
  const updateCreditForm = (field, value) => setCreditForm((current) => ({ ...current, [field]: ["monthlyPayment", "remainingDebt"].includes(field) ? formatNumberInput(value) : value }));
  ...
  const addOrUpdateAsset = (assetKey, form, editingId, resetForm, forcedCurrency = null) => {
    ...
    const payload = {
      ...
      note: form.note.trim(),
      // goldType yalnızca altın/metal kayıtlarında anlamlı; diğer türlerde "• GRAM" sızmasın.
      ...(assetKey === "gold" ? { goldType: form.goldType || "GRAM" } : {}),
    };
  ...
  const googleButton = (
    <>
      <div style={{ display: "flex", alignItems: "center", gap: 10, margin: "14px 0" }}>
        ...
        <span style={{ color: "#94a3b8", fontSize: 11, fontWeight: 700 }}>VEYA</span>
        <span style={{ flex: 1, height: 1, background: "rgba(255,255,255,.14)" }} />
      </div>
      <button
        type="button"
        onClick={handleGoogleLogin}
        style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, padding: "12px 16px", borderRadius: 14, border: "1px solid rgba(255,255,255,.22)", background: "#fff", color: "#1f2937", fontWeight: 800, fontSize: 14, cursor: "pointer" }}
      >
        <svg width="18" height="18" viewBox="0 0 48 48"><path fill="#FFC107" d="M43.6 20.1H42V20H24v8h11.3c-1.6 4.7-6.1 8-11.3 8-6.6 0-12-5.4-12-12s5.4-12 12-12c3.1 0 5.9 1.2 8 3l5.7-5.7C34 6.1 29.3 4 24 4 13 4 4 13 4 24s9 20 20 20 20-9 20-20c0-1.3-.1-2.6-.4-3.9z"/><path fill="#FF3D00" d="m6.3 14.7 6.6 4.8C14.7 15.1 19 12 24 12c3.1 0 5.9 1.2 8 3l5.7-5.7C34 6.1 29.3 4 24 4 16.3 4 9.7 8.3 6.3 14.7z"/><path fill="#4CAF50" d="M24 44c5.2 0 9.9-2 13.4-5.2l-6.2-5.2C29.2 35.1 26.7 36 24 36c-5.2 0-9.6-3.3-11.3-8l-6.5 5C9.5 39.6 16.2 44 24 44z"/><path fill="#1976D2" d="M43.6 20.1H42V20H24v8h11.3c-.8 2.2-2.2 4.2-4.1 5.6l6.2 5.2C36.9 39.2 44 34 44 24c0-1.3-.1-2.6-.4-3.9z"/></svg>
        Google ile devam et
      </button>
    </>
  );
...
function AssetList({ showPnl, items, onEdit, onDelete }) {
