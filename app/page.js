...
export default function HomePage() {
  ...
  const investmentTotals = useMemo(() => {
    ...
    return {
      ...
      availableInvestment: marketValue,
    };
      ...
      <div className="financeShell">
        ...
        {activeTab === "investments" ? (
          <InvestmentsTab
            ...
            forexOpen={forexOpen}
            setForexOpen={setForexOpen}
            forexForm={forexForm}
            updateForexForm={updateForexForm}
            addOrUpdateForex={addOrUpdateForex}
            editingForexId={editingForexId}
            resetForexForm={resetForexForm}
            startEditForex={startEditForex}
            investments={investments}
            setInvestments={setInvestments}
            setBesProjectionTotal={setBesProjectionTotal}
          />
        ) : null}
      ...
      <section className="authCard premiumAuthCard">
        ...
        <div className="authBrandRow">
          ...
          <div>
            ...
            <div className="authBrandSub">{authMode === "verify" ? "OTP doğrulama ekranı" : authMode === "register" ? "Yeni hesap oluştur" : "Güvenli kullanıcı paneli"}</div>
          </div>
        </div>
        ...
        {authMode === "verify" ? (
          <>
            <label className="authInputBox fullAuthInput">
              ...
              <input value={verificationCode} placeholder="Mailine gelen kod" onChange={(event) => setVerificationCode(event.target.value)} />
            </label>
            {authMessage ? <div className="authMessage">{authMessage}</div> : null}
            <div className="authButtons">
              <button type="button" className="premiumButton authPrimaryButton" onClick={handleVerifyCode}>Kodu Doğrula</button>
              <button type="button" className="secondaryButton authSecondaryButton" onClick={handleResendCode}>Kodu Tekrar Gönder</button>
              <button type="button" className="linkButton authBackButton" onClick={() => setAuthMode("login")}>Giriş ekranına dön</button>
            </div>
          </>
        ) : authMode === "register" ? (
          <>
            ...
            {authMessage ? <div className="authMessage">{authMessage}</div> : null}
            <div className="authButtons">
              <button type="button" className="premiumButton authPrimaryButton" onClick={handleRegister}>Kaydı Oluştur</button>
              <button type="button" className="secondaryButton authSecondaryButton" onClick={() => setAuthMode("login")}>Giriş Ekranına Dön</button>
            </div>
          </>
        ) : (
          <>
            ...
            {authMessage ? <div className="authMessage">{authMessage}</div> : null}
            <div className="authButtons">
              <button type="button" className="premiumButton authPrimaryButton" onClick={handleLogin}>Giriş Yap</button>
              <button type="button" className="secondaryButton authSecondaryButton" onClick={() => setAuthMode("register")}>Hesap Oluştur</button>
            </div>
          </>
        )}
  ...
  return (
    <>
      <section className="summaryGrid">
        <SummaryCard tone="green" title="Toplam Gelir" value={money(p.financeTotals.totalIncome)} detail={`Yemek parası: ${money(p.financeTotals.mealAllowance)} • Gelire dahil değil`} />
        <SummaryCard tone="red" title="Toplam Gider" value={money(p.financeTotals.totalExpense)} detail="Kredi + kart + diğer giderler" />
        <SummaryCard tone="blue" title="Aylık Kalan" value={money(p.financeTotals.balance)} detail="Gelir - gider hesabı" />
        <SummaryCard tone="purple" title="Toplam Borç" value={money(p.financeTotals.totalDebt)} detail="Kredi + kart + diğer borçlar" />
      </section>
      ...
      <Panel title="Gelirler" subtitle="Maaş, yemek parası ve ek gelir yönetimi" totalLabel="Toplam Gelir" total={money(p.financeTotals.totalIncome)} open={p.incomeOpen} onToggle={() => p.setIncomeOpen((value) => !value)}>
        ...
        <MiniPanel title="Ek Gelirler" totalLabel="Total Ek Gelir" total={money(p.financeTotals.extraIncomeTotal)} color="mint" open={p.extraIncomeOpen} onToggle={() => p.setExtraIncomeOpen((value) => !value)}>
          <div className="formGrid three">
            ...
            <button type="button" className="premiumButton" onClick={p.addOrUpdateExtraIncome}>{p.editingExtraIncomeId ? "Ek Geliri Güncelle" : "Ek Gelir Ekle"}</button>
          </div>
      ...
      <Panel title="Giderler" subtitle="Krediler, kredi kartı ve diğer borç/giderler" totalLabel="Toplam Gider" total={money(p.financeTotals.totalExpense)} open={p.expensesOpen} onToggle={() => p.setExpensesOpen((value) => !value)}>
        <MiniPanel title="Krediler" totalLabel="Total Kredi Ödemesi" total={money(p.financeTotals.activeCreditTotal)} color="purple" open={p.creditsOpen} onToggle={() => p.setCreditsOpen((value) => !value)}>
          ...
          <div className="formGrid five">
            ...
            <button type="button" className="premiumButton wide" onClick={p.addOrUpdateCredit}>{p.editingCreditId ? "Krediyi Güncelle" : "Kredi Ekle"}</button>
          </div>
        ...
        <MiniPanel title="Kredi Kartı Giderleri" totalLabel="Total Kredi Kartı" total={money(p.financeTotals.cardTotal)} color="rose" open={p.cardsOpen} onToggle={() => p.setCardsOpen((value) => !value)}>
          <SimpleExpenseForm form={p.cardForm} onChange={p.updateCardForm} onAdd={() => p.addOrUpdateSimpleExpense("card")} buttonText={p.editingCardId ? "Kart Giderini Güncelle" : "Kart Gideri Ekle"} />
          {p.editingCardId ? <button type="button" className="deleteButton" onClick={p.resetCardForm}>Düzenlemeyi İptal Et</button> : null}
          <SimpleExpenseList items={p.cardExpenses} money={money} onEdit={p.startEditCard} onDelete={(id) => p.setCardExpenses((current) => current.filter((item) => item.id !== id))} />
        </MiniPanel>

        <MiniPanel title="Diğer Borç / Nakit Giderler" totalLabel="Total Diğer" total={money(p.financeTotals.otherTotal)} color="orange" open={p.othersOpen} onToggle={() => p.setOthersOpen((value) => !value)}>
          <SimpleExpenseForm form={p.otherForm} onChange={p.updateOtherForm} onAdd={() => p.addOrUpdateSimpleExpense("other")} buttonText={p.editingOtherId ? "Diğer Gideri Güncelle" : "Diğer Gider Ekle"} />
          {p.editingOtherId ? <button type="button" className="deleteButton" onClick={p.resetOtherForm}>Düzenlemeyi İptal Et</button> : null}
          <SimpleExpenseList items={p.otherExpenses} money={money} onEdit={p.startEditOther} onDelete={(id) => p.setOtherExpenses((current) => current.filter((item) => item.id !== id))} />
        </MiniPanel>
      </Panel>
    </>
  );
...
function InvestmentsTab(p) {
  const [investmentFilter, setInvestmentFilter] = useState("tum");
  ...
  return (
    <>
      ...
      <section className="summaryGrid investmentSummaryGrid">
        <SummaryCard tone="green" title="Toplam Yatırım" value={money(p.investmentTotals.totalInvestment)} detail="BES dahil toplam yatırım" />
        <SummaryCard tone="purple" title="Blokajlı Yatırım" value={money(p.investmentTotals.blockedTotal)} detail="BES Projeksiyon toplamı" />
        <SummaryCard tone="blue" title="Kullanılabilir" value={money(p.investmentTotals.availableInvestment)} detail="BES hariç altın + hisse + kripto + döviz" />
      ...
      <Panel title="Yatırım Geliri" subtitle="Altın, hisse, kripto ve döviz takip paneli" totalLabel="BES Hariç Toplam" total={money(p.investmentTotals.availableInvestment)} open={p.investmentOpen} onToggle={() => p.setInvestmentOpen((value) => !value)}>
        <InvestmentFilter value={investmentFilter} onChange={setInvestmentFilter} />

        <MiniPanel title="Altın Yatırımı" totalLabel="Altın Değeri" total={money(p.investmentTotals.goldValue)} color="orange" open={p.goldOpen} onToggle={() => p.setGoldOpen((value) => !value)}>
          <AssetForm form={p.goldForm} onChange={p.updateGoldForm} buttonText={p.editingGoldId ? "Altını Güncelle" : "Altın Ekle"} onAdd={p.addOrUpdateGold} nameLabel="Altın Adı" namePlaceholder="Örn: Gram Altın" showGoldType />
          {p.editingGoldId ? <button type="button" className="deleteButton" onClick={p.resetGoldForm}>Düzenlemeyi İptal Et</button> : null}
          <AssetList items={p.investments.gold} filterMode={investmentFilter} onEdit={p.startEditGold} onDelete={(id) => p.setInvestments((current) => ({ ...current, gold: current.gold.filter((item) => item.id !== id) }))} />
        </MiniPanel>

        <MiniPanel title="Hisse Yatırımı" totalLabel="Hisse Değeri" total={money(p.investmentTotals.stockValue)} color="mint" open={p.stockOpen} onToggle={() => p.setStockOpen((value) => !value)}>
          <AssetForm form={p.stockForm} onChange={p.updateStockForm} buttonText={p.editingStockId ? "Hisseyi Güncelle" : "Hisse Ekle"} onAdd={p.addOrUpdateStock} nameLabel="Hisse / Sembol" namePlaceholder="Örn: THYAO / AAPL" />
          {p.editingStockId ? <button type="button" className="deleteButton" onClick={p.resetStockForm}>Düzenlemeyi İptal Et</button> : null}
          <AssetList items={p.investments.stocks} filterMode={investmentFilter} onEdit={p.startEditStock} onDelete={(id) => p.setInvestments((current) => ({ ...current, stocks: current.stocks.filter((item) => item.id !== id) }))} />
        </MiniPanel>

        <MiniPanel title="Kripto Yatırımı" totalLabel="Kripto Değeri" total={money(p.investmentTotals.cryptoValue)} color="rose" open={p.cryptoOpen} onToggle={() => p.setCryptoOpen((value) => !value)}>
          <AssetForm form={p.cryptoForm} onChange={p.updateCryptoForm} buttonText={p.editingCryptoId ? "Kriptoyu Güncelle" : "Kripto Ekle"} onAdd={p.addOrUpdateCrypto} nameLabel="Coin / Token" namePlaceholder="Örn: BTC / SOL" />
          {p.editingCryptoId ? <button type="button" className="deleteButton" onClick={p.resetCryptoForm}>Düzenlemeyi İptal Et</button> : null}
          <AssetList items={p.investments.crypto} filterMode={investmentFilter} onEdit={p.startEditCrypto} onDelete={(id) => p.setInvestments((current) => ({ ...current, crypto: current.crypto.filter((item) => item.id !== id) }))} />
        </MiniPanel>

        <MiniPanel title="Döviz Yatırımı" totalLabel="Döviz Değeri" total={money(p.investmentTotals.forexValue)} color="purple" open={p.forexOpen} onToggle={() => p.setForexOpen((value) => !value)}>
          <AssetForm form={p.forexForm} onChange={p.updateForexForm} buttonText={p.editingForexId ? "Dövizi Güncelle" : "Döviz Ekle"} onAdd={p.addOrUpdateForex} nameLabel="Döviz / Parite" namePlaceholder="Örn: USD Nakit / EUR" />
          {p.editingForexId ? <button type="button" className="deleteButton" onClick={p.resetForexForm}>Düzenlemeyi İptal Et</button> : null}
          <AssetList items={p.investments.forex} filterMode={investmentFilter} onEdit={p.startEditForex} onDelete={(id) => p.setInvestments((current) => ({ ...current, forex: current.forex.filter((item) => item.id !== id) }))} />
        </MiniPanel>
      </Panel>
...
function InvestmentFilter({ value, onChange }) {
  return (
    <div className="formGrid two">
      <SelectBox
        label="Yatırım Filtresi"
        value={value}
        onChange={onChange}
        options={[
          { value: "tum", label: "Tüm yatırımlar" },
          { value: "karda", label: "Sadece kârda olanlar" },
          { value: "zararda", label: "Sadece zararda olanlar" },
          { value: "degeriOlan", label: "Güncel değeri girilenler" },
          { value: "degeriOlmayan", label: "Güncel değeri girilmeyenler" },
        ]}
      />
      <div className="emptyState">
        <strong>Filtre</strong>
        <span>Yatırımları kâr, zarar veya güncel değer durumuna göre süzebilirsin.</span>
      </div>
    </div>
  );
}

function filterAssets(items, filterMode = "tum") {
  return [...items].filter((item) => {
    const cost = assetCostTry(item);
    const value = assetValueTry(item);
    const pnl = value - cost;
    const hasValue = value > 0;
    if (filterMode === "karda") return cost > 0 && hasValue && pnl >= 0;
    if (filterMode === "zararda") return cost > 0 && hasValue && pnl < 0;
    if (filterMode === "degeriOlan") return hasValue;
    if (filterMode === "degeriOlmayan") return !hasValue;
    return true;
  });
}
...
function SimpleExpenseForm({ form, onChange, onAdd, buttonText }) {
  return <div className="formGrid four"><InputBox label="Gider Adı" value={form.title} placeholder="Örn: Market" onChange={(value) => onChange("title", value)} /><InputBox label="Kategori" value={form.category} placeholder="Örn: Alışveriş" onChange={(value) => onChange("category", value)} /><InputBox label="Tutar" value={form.amount} placeholder="0" onChange={(value) => onChange("amount", value)} /><InputBox label="Not" value={form.note} placeholder="Opsiyonel" onChange={(value) => onChange("note", value)} /><button type="button" className="premiumButton wide" onClick={onAdd}>{buttonText}</button></div>;
}

function AssetForm({ form, onChange, onAdd, buttonText, nameLabel, namePlaceholder, showGoldType = false }) {
  return <><p className="sectionDescription">Alış fiyatı ve güncel fiyat zorunlu değildir. USD bazlı kayıtlar özetlerde güncel USD/TRY kuru ile TL’ye çevrilir.</p><div className="formGrid six"><InputBox label={nameLabel} value={form.title} placeholder={namePlaceholder} onChange={(value) => onChange("title", value)} />{showGoldType ? <SelectBox label="Altın Türü" value={form.goldType || "GRAM"} onChange={(value) => onChange("goldType", value)} options={goldTypeOptions} /> : null}<InputBox label="Adet / Miktar" value={form.quantity} placeholder="Örn: 0,63" onChange={(value) => onChange("quantity", value)} /><SelectBox label="Para Birimi" value={form.currency} onChange={(value) => onChange("currency", value)} options={currencyOptions} /><InputBox label="Alış Fiyatı" value={form.buyPrice} placeholder="Opsiyonel" onChange={(value) => onChange("buyPrice", value)} /><InputBox label="Güncel Fiyat" value={form.currentPrice} placeholder="Opsiyonel" onChange={(value) => onChange("currentPrice", value)} /><InputBox label="Not" value={form.note} placeholder="Opsiyonel" onChange={(value) => onChange("note", value)} /><button type="button" className="premiumButton wide" onClick={onAdd}>{buttonText}</button></div></>;
}
