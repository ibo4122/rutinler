...
export default function HomePage() {
  ...
  const investmentTotals = useMemo(() => {
    ...
    return { besTotal, lockedTotal: oldLockedTotal, blockedTotal, goldValue, cryptoValue, totalInvestment, totalPnl, totalPnlRate: totalCost > 0 ? (totalPnl / totalCost) * 100 : 0, availableInvestment: totalInvestment - blockedTotal };
  ...
  const startEditGold = (item) => {
    ...
    setGoldForm({ title: item.title || "", quantity: String(item.quantity || ""), buyPrice: formatNumberInput(item.buyPrice), currentPrice: formatNumberInput(item.currentPrice), note: item.note || "" });
  ...
  const startEditCrypto = (item) => {
    ...
    setCryptoForm({ title: item.title || "", quantity: String(item.quantity || ""), buyPrice: formatNumberInput(item.buyPrice), currentPrice: formatNumberInput(item.currentPrice), note: item.note || "" });
    setInvestmentOpen(true);
    setCryptoOpen(true);
  };
