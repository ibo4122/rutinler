...
function assetValueTry(item) {
  const quantity = Number(item?.quantity || 0);
  const explicitCurrentPriceTry = Number(item?.currentPriceTry || 0);

  if (quantity > 0 && explicitCurrentPriceTry > 0) {
    return quantity * explicitCurrentPriceTry;
  }
...
function assetCostTry(item) {
  const quantity = Number(item?.quantity || 0);
  const explicitBuyPriceTry = Number(item?.buyPriceTry || 0);

  if (quantity > 0 && explicitBuyPriceTry > 0) {
    return quantity * explicitBuyPriceTry;
  }
...
function normalizeFinanceData(data) {
  ...
  return {
    ...
    investments: {
      ...
      gold: Array.isArray(data.investments?.gold)
        ? data.investments.gold.map((item) => normalizeAsset(item, "TRY"))
        : [],
      crypto: Array.isArray(data.investments?.crypto)
        ? data.investments.crypto.map((item) => normalizeAsset(item, "USD"))
        : [],
      stocks: Array.isArray(data.investments?.stocks)
        ? data.investments.stocks.map((item) => normalizeAsset(item, "TRY"))
        : [],
      forex: Array.isArray(data.investments?.forex)
        ? data.investments.forex.map((item) => normalizeAsset(item, "USD"))
        : [],
    },
...
export default function HomePage() {
  ...
  const handleVerifyCode = async () => {
    ...
    if (firstTry.error) {
      ...
      if (secondTry.error) {
        return setAuthMessage("Kod doğrulanamadı. Kodu kontrol et.");
      }
    }
  ...
  const handleLogin = async () => {
    ...
    if (error) {
      return setAuthMessage("Giriş başarısız. E-posta doğrulanmamış olabilir veya şifre hatalı olabilir.");
    }
  ...
  const financeTotals = useMemo(() => {
    ...
    const activeCreditTotal = credits.reduce((sum, item) => {
      return isCreditActive(item.paymentStartDate) ? sum + Number(item.monthlyPayment || 0) : sum;
    }, 0);
  ...
  const investmentTotals = useMemo(() => {
    ...
    const blockedTotal = besTotal;
    const totalInvestment = marketValue + besTotal;
  ...
  const updateAssetForm = (setter) => (field, value) => {
    setter((current) => ({
      ...current,
      [field]: value,
    }));
  ...
  const resetForexForm = () => {
    setEditingForexId(null);
    setForexForm({
      ...emptyAssetForm,
      currency: "USD",
    });
  ...
  const addOrUpdateExtraIncome = () => {
    ...
    if (!title || amount <= 0) {
      return alert("Ek gelir adı ve geçerli tutar gir.");
    }

    if (editingExtraIncomeId) {
      setExtraIncomes((current) =>
        current.map((item) => (item.id === editingExtraIncomeId ? { ...item, title, amount } : item))
      );
  ...
  const addOrUpdateCredit = () => {
    ...
    if (editingCreditId) {
      setCredits((current) =>
        current.map((item) => (item.id === editingCreditId ? { ...item, ...payload } : item))
      );
      resetCreditForm();
      return;
    }
  ...
  const addOrUpdateSimpleExpense = (type) => {
    ...
    if (!title || amount <= 0) {
      return alert("Gider adı ve geçerli tutar gir.");
    }
    ...
    if (isCard) setCardForm(emptyExpense);
    else setOtherForm(emptyExpense);
  };
  ...
  const addOrUpdateAsset = (assetKey, form, editingId, resetForm) => {
    ...
    if (!title) {
      return alert("Varlık adı / sembol gir.");
    }
    ...
    <main className="financePage">
      <div className="financeShell">
        ...
        <div className="tabBar">
          ...
          <button
            type="button"
            className={activeTab === "routines" ? "tabButton active" : "tabButton"}
        ...
        {authMode === "verify" ? (
          <>
            ...
            <div className="authButtons">
              <button type="button" className="premiumButton authPrimaryButton" onClick={handleVerifyCode}>
                Kodu Doğrula
              </button>
              <button type="button" className="secondaryButton authSecondaryButton" onClick={handleResendCode}>
                Kodu Tekrar Gönder
              </button>
              <button type="button" className="linkButton authBackButton" onClick={() => setAuthMode("login")}>
                Giriş ekranına dön
              </button>
            </div>
          </>
    ...
    <>
      ...
      >
        <div className="formGrid two">
          <InputBox
            label="Maaş"
            value={p.income.salary}
            placeholder="0"
            onChange={(value) => p.updateIncome("salary", value)}
          />
          <InputBox
            label="Yemek Parası"
            value={p.income.mealAllowance}
            placeholder="0"
            onChange={(value) => p.updateIncome("mealAllowance", value)}
          />
        </div>
        ...
        >
          <div className="formGrid three">
            <InputBox
              label="Ek Gelir Adı"
              value={p.extraIncomeForm.title}
              placeholder="Örn: Prim"
              onChange={(value) => p.updateExtraIncomeForm("title", value)}
            />
            <InputBox
              label="Tutar"
              value={p.extraIncomeForm.amount}
              placeholder="0"
              onChange={(value) => p.updateExtraIncomeForm("amount", value)}
            />
  ...
  return (
    <>
      ...
      <Panel
        ...
        subtitle="Altın, hisse, kripto, döviz ve BES takip paneli"
      ...
      >
        ...
        <MiniPanel
          title="Hisse Yatırımı"
          totalLabel="Hisse Değeri"
          total={money(p.investmentTotals.stockValue)}
          color="mint"
          open={p.stockOpen}
          onToggle={() => p.setStockOpen((value) => !value)}
        >
          <AssetForm
            form={p.stockForm}
            onChange={p.updateStockForm}
            buttonText={p.editingStockId ? "Hisseyi Güncelle" : "Hisse Ekle"}
            onAdd={p.addOrUpdateStock}
            nameLabel="Hisse / Sembol"
            namePlaceholder="Örn: THYAO / AAPL"
          />

          {p.editingStockId ? (
            <button type="button" className="deleteButton" onClick={p.resetStockForm}>
              Düzenlemeyi İptal Et
            </button>
          ) : null}

          <SortControl value={stockSort} onChange={setStockSort} />

          <AssetList
            items={p.investments.stocks}
            sortMode={stockSort}
            onEdit={p.startEditStock}
            onDelete={(id) =>
              p.setInvestments((current) => ({
                ...current,
        ...
        >
          ...
          <SortControl value={cryptoSort} onChange={setCryptoSort} />
        ...
        <MiniPanel
          title="Döviz Yatırımı"
          totalLabel="Döviz Değeri"
          total={money(p.investmentTotals.forexValue)}
          color="purple"
          open={p.forexOpen}
          onToggle={() => p.setForexOpen((value) => !value)}
        >
          <AssetForm
            form={p.forexForm}
            onChange={p.updateForexForm}
            buttonText={p.editingForexId ? "Dövizi Güncelle" : "Döviz Ekle"}
            onAdd={p.addOrUpdateForex}
            nameLabel="Döviz / Parite"
            namePlaceholder="Örn: USD Nakit / EUR"
          />

          {p.editingForexId ? (
            <button type="button" className="deleteButton" onClick={p.resetForexForm}>
              Düzenlemeyi İptal Et
            </button>
          ) : null}

          <AssetList
            items={p.investments.forex}
            onEdit={p.startEditForex}
            onDelete={(id) =>
              p.setInvestments((current) => ({
                ...current,
                forex: current.forex.filter((item) => item.id !== id),
              }))
            }
          />
        </MiniPanel>
  ...
  return (
    <div className="formGrid two">
      <SelectBox
        ...
        onChange={onChange}
        options={[
          { value: "valueDesc", label: "Değer büyükten küçüğe" },
          { value: "valueAsc", label: "Değer küçükten büyüğe" },
          { value: "pnlDesc", label: "Kar/Zarar büyükten küçüğe" },
          { value: "pnlAsc", label: "Kar/Zarar küçükten büyüğe" },
          { value: "profitFirst", label: "Kârda olanlar önce" },
          { value: "lossFirst", label: "Zararda olanlar önce" },
          { value: "nameAsc", label: "İsme göre" },
      ...
      <div className="emptyState">
        ...
        <span>Değer, kar/zarar, isim veya eklenme tarihine göre sıralayabilirsin.</span>
      </div>
    </div>
  );
}
...
function AssetForm({
  ...
  return (
    <>
      <p className="sectionDescription">
        Alış fiyatı ve güncel fiyat zorunlu değildir. USD bazlı kayıtlar özetlerde güncel USD/TRY kuru ile TL’ye çevrilir.
      </p>
      ...
      <div className="formGrid six">
        ...
        {showGoldType ? (
          <SelectBox
            label="Altın Türü"
            value={form.goldType || "GRAM"}
            onChange={(value) => onChange("goldType", value)}
...
function AssetList({ items, sortMode = "valueDesc", onEdit, onDelete }) {
  if (items.length === 0) return <EmptyState text="Henüz yatırım kaydı bulunmuyor." />;

  return (
    <div className="recordList">
      {sortAssets(items, sortMode).map((item) => {
        ...
        const recordStyle =
          hasCost && hasValue
            ? {
                borderColor: isProfit ? "rgba(34, 197, 94, 0.55)" : "rgba(248, 113, 113, 0.55)",
                background: isProfit ? "rgba(34, 197, 94, 0.10)" : "rgba(248, 113, 113, 0.10)",
              }
            : undefined;

        return (
          <div key={item.id} className="simpleRecord investmentRecord" style={recordStyle}>
            <div className="simpleRecordTop">
              <div>
                <h4>{item.title}</h4>
                <p>
                  Miktar: {item.quantity || 0} • Para birimi: {item.currency || "TRY"}
                  {item.goldTypeLabel ? ` • ${item.goldTypeLabel}` : ""}
                </p>
                <p>
                  Alış: {item.buyPrice ? formatCurrency(item.buyPrice, item.currency) : "Girilmedi"} • Güncel:{" "}
                  {item.currentPrice ? formatCurrency(item.currentPrice, item.currency) : "Girilmedi"}
                </p>
                <p>
                  TL Maliyet: {hasCost ? money(cost) : "Hesaplanmadı"} • TL Güncel Değer:{" "}
                  {hasValue ? money(value) : "Hesaplanmadı"}
                </p>
                {item.usdTryRate ? <p>USD/TRY: {Number(item.usdTryRate).toFixed(4)}</p> : null}
                {item.livePriceSource ? <p>Canlı kaynak: {item.livePriceSource}</p> : null}
                {item.note ? <p>{item.note}</p> : null}
              </div>

              <div className="simpleRecordRight">
                <strong className={isProfit ? "profitText" : "lossText"}>
                  {hasCost && hasValue ? `${money(pnl)} / %${pnlRate.toFixed(2)}` : "Kar/Zarar yok"}
                </strong>
