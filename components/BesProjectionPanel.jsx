  ...
  const projection = useMemo(() => {
    ...
    for (let month = 1; month <= modelTotalMonths; month += 1) {
      ...
      rows.push({
        ...
        stateFundClosing,
        vestingRate: month >= critical ? vestRate : 0,
        earnedStateAmount,
      ...
      stateFundOpening = stateFundClosing;
    }
      ...
      <button type="button" className="panelHeader">
        ...
        <div className="panelRight">
          <div className="panelTotal">
            <span>Çıkış Portföyü</span>
            <strong>{money(projection.exitRow?.totalPortfolioValue)}</strong>
          </div>
        </div>
      </button>
      ...
      <div className="panelBody">
        <section className="summaryGrid investmentSummaryGrid">
          ...
          <article className="summaryCard purple">
            <div className="summaryLabel">5. Yıl Sonu</div>
            <div className="summaryValue summaryValue-purple">
              {money(projection.criticalRow?.totalPortfolioValue)}
            </div>
            <div className="summaryDetail">Aralık 2029 kritik eşik</div>
          </article>

          <article className="summaryCard green">
            <div className="summaryLabel">6. Yıl Sonu</div>
            <div className="summaryValue summaryValue-green">
              {money(projection.exitRow?.totalPortfolioValue)}
            </div>
            <div className="summaryDetail">Aralık 2030 çıkış senaryosu</div>
          </article>

          <article className="summaryCard red">
            <div className="summaryLabel">Hak Edilen Devlet</div>
            <div className="summaryValue summaryValue-red">
              {money(projection.exitRow?.earnedStateAmount)}
            </div>
            <div className="summaryDetail">%{vestingRate} hak ediş varsayımı</div>
          </article>
        </section>

        <section className="miniPanel purple">
          <div className="miniHeader">
            <div>
              <h3 className="miniTitle miniTitle-purple">Dinamik Girdiler</h3>
            </div>
          </div>
              ...
              <tbody>
                {projection.rows.map((row) => (
                  ...
                  >
                    ...
                    <td>{money(row.monthlyStateContribution)}</td>
                    <td>{money(row.monthlyStateFundReturn)}</td>
