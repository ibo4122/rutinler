    ...
    <section className="panelCard besProjectionPanel">
      <button type="button" className="panelHeader" onClick={() => setMainOpen((value) => !value)}>
        <div>
          ...
          <p>Mevcut BES değeri, yukarıdaki Blokajlı Yatırım toplamına otomatik eklenir.</p>
        </div>
        <div className="panelRight">
          <div className="panelTotal"><span>Mevcut Total Para</span><strong>{money(projection.currentTotalMoney)}</strong></div>
          <div className="panelTotal"><span>İlerleme</span><strong>{projection.currentMonth} / {projection.modelTotalMonths} Ay</strong></div>
      ...
      {mainOpen ? (
        <div className="panelBody">
          <div className="besProgressCard">
            <div className="besProgressTop"><div><span>Mevcut Ay</span><strong>{projection.currentMonth}. Ay</strong></div><div><span>Toplam Süre</span><strong>{projection.modelTotalMonths} Ay</strong></div><div><span>Tamamlanma</span><strong>%{projection.progressPercent.toFixed(1)}</strong></div></div>
            <div className="besProgressTrack"><div className="besProgressFill" style={{ width: `${projection.progressPercent}%` }} /></div>
          </div>

          <section className="summaryGrid investmentSummaryGrid">
            <article className="summaryCard green"><div className="summaryLabel">Mevcut Total Para</div><div className="summaryValue summaryValue-green">{money(projection.currentTotalMoney)}</div><div className="summaryDetail">Ana para + fon getirisi + devlet toplamının %{vestingRate} payı</div></article>
            ...
            <article className="summaryCard purple"><div className="summaryLabel">Mevcut Fon Getirisi</div><div className="summaryValue summaryValue-purple">{money(projection.currentRow?.cumulativeFundReturn)}</div><div className="summaryDetail">Gerçekleşen ana fon kârı</div></article>
            <article className="summaryCard red"><div className="summaryLabel">Hak Edilen Devlet Payı</div><div className="summaryValue summaryValue-red">{money(projection.currentEarnedStateAmount)}</div><div className="summaryDetail">Devlet katkısı + devlet fon getirisi x %{vestingRate}</div></article>
          </section>
